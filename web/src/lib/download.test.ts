...
import { classifyDownloads, missingFromLibrary, unwatchedOnServerCount, type DownloadShow } from "./download";

// The episode row shape missingFromLibrary consumes (ProgressEpisode + title).
...
describe("missingFromLibrary", () => {
  const today = "2026-07-11";

  it("returns aired, unwatched episodes that aren't on the server", () => {
    const eps = [
      ep("watched", 1, 1, "2026-01-01"),
      ep("owned", 1, 2, "2026-02-01"),
      ep("missing", 1, 3, "2026-03-01"),
      ep("future", 1, 4, "2027-01-01"),
    ];
    const missing = missingFromLibrary(eps, new Set(["watched"]), new Set(["owned"]), today);
  ...
  it("excludes specials, mirroring the counted-episode rule", () => {
    ...
    expect(missingFromLibrary(eps, new Set(), new Set(), today).map((m) => m.id)).toEqual(["regular"]);
  });

  it("treats a null or future release date as not aired", () => {
    const eps = [ep("undated", 1, 1, null), ep("future", 1, 2, "2999-01-01")];
    expect(missingFromLibrary(eps, new Set(), new Set(), today)).toEqual([]);
  });
  ...
  it("orders results by season then episode (earliest to download first)", () => {
    ...
    expect(missingFromLibrary(eps, new Set(), new Set(), today).map((m) => m.id)).toEqual(["c", "a", "b"]);
  });

  it("finds a missing later episode even when earlier ones of the same season are on the server", () => {
    ...
    const missing = missingFromLibrary(eps, new Set(), new Set(["e1", "e2"]), today);
  ...
  it("restricts to complete seasons when given a completeSeasons set (wait for the full season)", () => {
    ...
    expect(missingFromLibrary(eps, new Set(), new Set(), today, new Set([1])).map((m) => m.id)).toEqual(["s1e1"]);
    // An empty complete-season set (no season fully aired yet) means nothing is downloadable.
    expect(missingFromLibrary(eps, new Set(), new Set(), today, new Set())).toEqual([]);
  });
});

describe("unwatchedOnServerCount", () => {
  const today = "2026-07-11";

  it("counts aired, unwatched episodes that ARE on the server (the 'still to watch' count)", () => {
    const eps = [
      ep("watched", 1, 1, "2026-01-01"), // on the server but watched — doesn't count
      ep("toWatch1", 1, 2, "2026-02-01"), // on the server, unwatched — counts
      ep("toWatch2", 1, 3, "2026-03-01"), // on the server, unwatched — counts
      ep("missing", 1, 4, "2026-04-01"), // not on the server — doesn't count
      ep("future", 1, 5, "2027-01-01"), // on the server but not aired — doesn't count
    ];
    const present = new Set(["watched", "toWatch1", "toWatch2", "future"]);
    expect(unwatchedOnServerCount(eps, new Set(["watched"]), present, today)).toBe(2);
  });

  it("is 0 when every in-Plex episode is watched (the 'Get back' case)", () => {
    const eps = [ep("a", 1, 1, "2026-01-01"), ep("b", 1, 2, "2026-02-01")];
    // Both aired episodes are on the server and both watched; a later one (c) aired but isn't downloaded.
    const eps2 = [...eps, ep("c", 1, 3, "2026-03-01")];
    expect(unwatchedOnServerCount(eps2, new Set(["a", "b"]), new Set(["a", "b"]), today)).toBe(0);
  });

  it("excludes specials and not-yet-aired episodes", () => {
    const eps = [ep("special", 0, 1, "2026-01-01", true), ep("future", 1, 1, "2999-01-01")];
    expect(unwatchedOnServerCount(eps, new Set(), new Set(["special", "future"]), today)).toBe(0);
  });

  it("counts only complete seasons when given a completeSeasons set", () => {
    // Both on the server + unwatched, but season 2 is still airing → only season 1's episode counts as watchable now.
    const eps = [ep("s1e1", 1, 1, "2026-01-01"), ep("s2e1", 2, 1, "2026-06-01")];
    expect(unwatchedOnServerCount(eps, new Set(), new Set(["s1e1", "s2e1"]), today, new Set([1]))).toBe(1);
  });
});
...
describe("classifyDownloads", () => {
  ...
  const analyzed = (title: string, lastWatchedAt: string | null, onServerLeft: number) => ({
