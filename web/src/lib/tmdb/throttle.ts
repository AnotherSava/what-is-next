// Global request throttle for the TMDB client (brief §3.2): stay under ~40 requests / 10 s, with a bounded
// concurrency so a bulk import (hundreds of season fetches) can't stampede. This is a sliding window over
// request STARTS: a request may begin only when fewer than `maxConcurrent` are in flight AND fewer than
// `maxRequests` have started within the last `windowMs`. Clock + sleep are injectable so tests are deterministic.
